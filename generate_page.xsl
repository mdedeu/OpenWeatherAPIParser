...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template match = "results">
         ...
         <xsl:when test="count(//error)>0">
           <h2> You Have Errors, Please Check weather_data.xml for more information</h2>
         </xsl:when>
         <xsl:otherwise>
         	<html>
			<head>
			...
			<body>
				<header>
					<nav>
						...
						<ul class= "nav-links">
							...
							<li > <a class= "nav-link" href="./extra/diez.html">¿Por qué deberíamos sacarnos un diez?</a></li>
							<li > <a class= "nav-link" href="./extra/nosotros.html">Nosotros</a></li>
						</ul>
					</nav>
				</header>
				...
				<xsl:for-each select = "//country">
					...
					<xsl:value-of select="count(./cities/city)"/><xsl:choose>
  																	<xsl:when test="count(./cities/city)=1">&#160;city in </xsl:when>
  																	<xsl:otherwise>&#160;cities in </xsl:otherwise>
  																	</xsl:choose> 
...
</xsl:stylesheet>				<td><xsl:value-of select = "./pressure"/>&#160;<xsl:value-of select = "./pressure/@unit"/></td>
								<td><xsl:value-of select = "./clouds"/></td>
								<td><xsl:value-of select = "./weather"/>&#160;<img ><xsl:attribute name="src">./images/icons/<xsl:value-of select="./weather/@icon"/>.png</xsl:attribute></img></td>
							</tr>
						</xsl:for-each>
